...
def reportBt(bt, benchmark="多因子模型", fubenchmark=''):
    ...
    if fubenchmark:
        data['benchmark_return'] = DataAPI.MktMFutdGet(mainCon=u"1", contractMark=u"L0", contractObject=fubenchmark,
                                                        startDate=bt['tradeDate'].iloc[0],
                                                        endDate=bt['tradeDate'].iloc[-1],
                                                        field=u"tradeDate,chgPct",
                                                        pandas="1").set_index('tradeDate')['chgPct']
        title = fubenchmark

    elif benchmark not in BENCHMARK_DICT:
        arr = []
        for day in bt['tradeDate']:
            yesterday = cal.advanceDate(day, '-1B').strftime("%Y-%m-%d")
            symbols_weights = get_index_data3(benchmark, yesterday, False)
        ...
        title = BENCHMARK_DICT[benchmark]
    ...
    data['hedged_current_drawdown'] = [-1 + v / max(1, max(data['excess'][:i + 1])) for i, v in
                                       enumerate(data['excess'])]  # 对冲后净值回撤
    ...
    ax1.plot(data['tradeDate'], data[['benchmark']], label="Index", color='b')
    ax1.plot(data['tradeDate'], data[['portfolio']], label="Portfolio", color='r')
    ax2.plot(data['tradeDate'], data[['excess']], label="Alpha", color='r')
    ax3.plot(data['tradeDate'], data['hedged_current_drawdown'], label="Drawdown", color='b')
    ...
    ax3.set_ylabel(u"回撤", fontproperties=font, fontsize=16)

    ax1.set_title(u"%s - 净值走势" % title, fontproperties=font, fontsize=16)
    ax2.set_title(u"%s - 对冲基准指数后净值走势" % title, fontproperties=font, fontsize=16)
    ax3.set_title(u"%s - 对冲基准指数后净值回撤" % title, fontproperties=font, fontsize=16)
...
def riskCoeffs(perf):
    ...
    perf['hedged_annualized_return'] = ex_cumreturn.values[-1] ** (252.0 / ex_cumreturn.size) - 1
